...
if grep -rn "<<<<<<< HEAD" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.json" --include="*.md" --exclude-dir=.gitnexus --exclude-dir=dist --exclude-dir=.next --exclude-dir=out --exclude-dir=build --exclude-dir=node_modules --exclude-dir=data --exclude-dir=public packages/ apps/ scripts/ 2>/dev/null; then
