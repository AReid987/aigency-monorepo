...
export const SurrealClient = {
  async connect(config: SurrealClientConfig): Promise<Surreal> {
    if (_instance) {
      return _instance;
    }
  ...
  get db(): Surreal {
    if (!_instance) {
      throw new Error("[surreal] Not connected. Call SurrealClient.connect() first.");
    }
    return _instance;
  },
  ...
  async reconnect(): Promise<Surreal> {
    if (!_config) {
      throw new Error("[surreal] No config to reconnect with.");
    }
    _instance = null;
